...
def get_asset_radius_and_bottom_offset(source_path):
    ...
    # bottom offset relative to pivot
    # final pivot z = ground z - bottom_offset
...
def sample_clustered_xy(bounds, centers, cluster_strength, cluster_radius):
    ...
    # cluster_strength 0 = random, 1 = grouped
...
async def generate_preview_async():
    ...
    for type_name, data in pcg_types.items():
        ...
        while len(type_points) < count and attempts < max_attempts:
            ...
            # This is the important ground-stick fix:
            # place pivot so asset bottom sits on ground
        ...
        if len(type_points) < count:
            print(f"⚠️ Only placed {len(type_points)}/{count} for {type_name}. Reduce Min Distance / Obstacle Padding.")

    set_status(f"✅ Grouped preview done. Ray hits: {ray_hits}, fallback: {fallbacks}", 1.0)
    is_processing = False
...
async def final_spawn_async():
    ...
    for i, item in enumerate(generated_points):
        ...
        set_status(f"Final spawning grouped assets... {i + 1}/{total}", (i + 1) / total)
    ...
    set_status(f"✅ Final grouped spawn done. Spawned: {spawned}, skipped: {skipped}", 1.0)
    is_processing = False
...
def build_gui():
    ...
    pcg_window = ui.Window("Exciting Isaac Sim PCG Scatter Tool", width=600, height=900)
    ...
    with pcg_window.frame:
        with ui.ScrollingFrame(
            ...
            with ui.VStack(spacing=10):
                ...
                ui.Label("Clustered nature groups, obstacle avoidance, and proper ground sticking.")
                ...
                with ui.CollapsableFrame(title="🚧 Obstacles To Avoid", collapsed=False):
                    with ui.VStack(spacing=6):
                        ui.Label("Select rocks/buildings/objects to avoid, then add them here.")
                ...
                with ui.CollapsableFrame(title="🚀 Actions + Progress", collapsed=False):
                    with ui.VStack(spacing=8):
                        ...
                        ui.Button("Generate Exciting Grouped Preview", height=40, clicked_fn=generate_preview)
                ...
                with ui.CollapsableFrame(title="📘 Usage Guide", collapsed=True):
                    with ui.VStack(spacing=4):
                        ui.Label("1. Select landscape/plane → Set Spawn Area.")
                        ui.Label("2. Select trees/stones/bushes → Add to matching group.")
                        ui.Label("3. Select objects to avoid → Add as Obstacles.")
                        ui.Label("4. Use Cluster Count, Strength, Radius for natural groups.")
                        ui.Label("5. Generate preview, then final spawn.")
                        ui.Label("Tip: If objects float/sink, adjust Height Offset slightly.")
...
print("✅ Exciting grouped PCG with obstacle avoidance loaded.")